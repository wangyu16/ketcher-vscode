...
import { embedDataInSvg, extractDataFromSvg } from './utils/svgHandler';

export class KetcherEditorProvider implements vscode.CustomTextEditorProvider {
    ...
    private static readonly viewType = 'ketcher.editor';
    ...
    private async updateWebview(webview: vscode.Webview, document: vscode.TextDocument) {
        ...
        const data = await extractDataFromSvg(text);
        if (data) {
            webview.postMessage({
                type: 'INIT',
                data: data
    ...
    // Phase 4: Data Sync
    private async saveDocument(document: vscode.TextDocument, data: { ket: any, svg: string }) {
        if (!data || !data.ket || !data.svg) { return; }

        // Embed the KetJSON into the SVG
        const newFileContent = await embedDataInSvg(data.svg, data.ket);
