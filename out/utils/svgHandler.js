...
exports.embedDataInSvg = embedDataInSvg;
exports.extractDataFromSvg = extractDataFromSvg;
const xml2js_1 = require("xml2js");
const KETCHER_DATA_ID = 'ketcher-data';
async function embedDataInSvg(svgContent, chemicalData) {
    let result;
    try {
        result = await (0, xml2js_1.parseStringPromise)(svgContent);
    }
    catch (e) {
...
async function extractDataFromSvg(svgContent) {
    try {
        const result = await (0, xml2js_1.parseStringPromise)(svgContent);
        if (!result.svg || !result.svg.metadata) {
            return null;
        }
        const metadata = result.svg.metadata.find((m) => m.$?.id === KETCHER_DATA_ID);
